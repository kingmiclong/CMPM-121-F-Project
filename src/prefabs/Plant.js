class Plant {
  ...
  updateSprite() {
    const frame = PlantSpecies.getFrameForSpecies(
      this.species,
      this.growthStage
    );
  ...
  checkGrowthConditions() {
    ...
    if (!speciesData) {
      console.error(
        "Growth conditions are undefined for species:",
        this.species
      );
      return;
    }
    ...
    if (
      currentSunlight >= speciesData.sun.min &&
      currentSunlight <= speciesData.sun.max &&
      currentWater >= speciesData.water.min &&
      currentWater <= speciesData.water.max
    ) {
      this.grow();
    }
